...
export const InternetSpeedTest = ({
  ...
  const isTestRunning =
    results.status !== 'Inactivo' &&
    results.status !== 'Finalizado' &&
    results.status !== 'Error' &&
    results.status !== 'Sin Conexión';
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.statsContainer}>
        ...
        <Animated.View
          ...
          {results.status === 'Probando Descarga' && (
            <ActivityIndicator size={14} color={Colors.primary} style={styles.cardLoader} />
          )}
        </Animated.View>
        ...
        {/* Ping */}
        <Animated.View style={[styles.statCard, { transform: [{ scale: pingPulse }] }]}>
      ...
      {results.coordinates && (
        <View style={styles.infoCard}>
          ...
          <Text style={styles.infoValue} numberOfLines={1} adjustsFontSizeToFit>
            Lat: {results.coordinates.latitude.toFixed(6)}, Lng:{' '}
      ...
      <View style={styles.statusContainer}>
        ...
        <Text style={[styles.statusValue, { color: getStatusColor() }]}>{results.status}</Text>
        {isTestRunning && <ActivityIndicator color={Colors.primary} style={styles.loader} />}
        {isFinished && <Text style={styles.successIcon}>✓</Text>}
