...
export default function PhotoEvidenceManager({
  ...
  const { uploadEvidence, deleteEvidence, getImageUrl, uploading, deleting, error } =
    useOrderEvidence();
  ...
  const showPickerOptions = () => {
    Alert.alert('Agregar Evidencia', 'Selecciona una opción', [
  ...
  const handleDelete = (photo: PhotoItem) => {
    Alert.alert('Eliminar Evidencia', '¿Estás seguro de eliminar esta foto?', [
      { text: 'Cancelar', style: 'cancel' },
      {
        text: 'Eliminar',
        style: 'destructive',
