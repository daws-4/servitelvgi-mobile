...
import type { AssignedInventoryItem, EquipmentInstance, InventoryItem, InventoryBatch } from '@/types/Inventory';
...
const getMaterialDescription = (
    ...
    const materialItemId = typeof material.item === 'string'
        ? material.item
        : (material.item as any)?._id;
...
export default function InventoryAssignmentManager({
    ...
    const handleAddItem = () => {
        ...
        if (isEquipment) {
            if (!selectedInstance) {
                Alert.alert('Error', 'Debe seleccionar una instancia específica (Serial/MAC) para este equipo');
                return;
            }
        }
        ...
        if (isBatchItem && selectedBatch) {
            if (qty > selectedBatch.currentQuantity) {
                Alert.alert('Error', `Solo hay ${selectedBatch.currentQuantity} ${selectedBatch.unit} disponibles en esta bobina`);
                return;
            }
                        ...
                        <View key={`${material.item}-${index}`} style={styles.materialRow}>
                            <View style={styles.materialInfo}>
                                <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6 }}>
                                    {material.batchCode && (
                                        <FontAwesome name="circle" size={8} color="#3b82f6" />
                                    )}
                                    {material.instanceIds && material.instanceIds.length > 0 && (
                                        <FontAwesome name="microchip" size={10} color="#9333ea" />
                                    )}
                                    <Text style={styles.materialName}>{getMaterialDescription(material, inventory, batches)}</Text>
                                </View>
                                <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8, marginTop: 2 }}>
                                    <Text style={styles.materialQty}>
                                        Cant: {material.quantity}
                                    </Text>
                                    {material.batchCode && (
                                ...
                                {material.instanceDetails && material.instanceDetails.length > 0 && (
                                    <View style={{ marginTop: 6, paddingLeft: 12 }}>
                                        {material.instanceDetails.map((detail: { uniqueId: string; serialNumber: string }, idx: number) => (
                ...
                <View style={styles.overlay}>
                    ...
                    >
                        ...
                        <View style={[styles.modalContent, { flex: 1 }]}>
                                            ...
                                            ListEmptyComponent={
                                                <View style={styles.centerContainer}>
                                                    <Text style={styles.emptyListText}>
                                                        {searchQuery ? 'No se encontraron resultados' : 'No hay inventario disponible'}
                                                    </Text>
                                                </View>
                                            }
                                        />
                                        ...
                                        <FlatList
                                            ...
                                            renderItem={({ item }) => (
                                                <TouchableOpacity
                                                    style={[
                                                        styles.instanceRow,
                                                        selectedInstance?.uniqueId === item.uniqueId && styles.selectedInstanceRow
                                                    ]}
                                            ...
                                            ListEmptyComponent={
                                                <View style={styles.centerContainer}>
                                                    <Text style={styles.emptyListText}>
                                                        {instanceSearchQuery ? 'No se encontraron resultados' : 'No hay instancias disponibles'}
                                                    </Text>
                                                </View>
                                            }
                                        />
                                        ...
                                        renderItem={({ item }) => (
                                            ...
                                            >
                                                <View style={styles.instanceInfo}>
                                                    <Text style={[
                                                        styles.instanceSerial,
                                                        selectedBatch?.batchCode === item.batchCode && styles.selectedInstanceText
                                                    ]}>
                                                        {item.batchCode}
                                                    </Text>
                                                    <Text style={[
                                                        styles.instanceMac,
                                                        selectedBatch?.batchCode === item.batchCode && styles.selectedInstanceSubtext
                                                    ]}>
                            ...
                            {selectedItem && (viewState === 'inventory' || selectedInstance || (viewState === 'batches' && selectedBatch)) && (
                                <Animated.View style={[styles.footer, { bottom: keyboardHeight }]}>
                                    <View style={styles.selectedSummary}>
                                        ...
                                        <Text style={styles.selectedValue} numberOfLines={1}>
                                            ...
                                            {selectedInstance ? ` (${selectedInstance.serialNumber || selectedInstance.uniqueId})` : ''}
                                        ...
                                        <View style={styles.quantityContainer}>
                                            ...
                                            <View style={styles.quantityControls}>
                                                ...
                                                <TouchableOpacity
                                                    style={styles.qtyBtn}
                                                    onPress={() => {
                                                        const max = selectedBatch ? selectedBatch.currentQuantity : selectedItem.quantity;
                                    ...
                                    <TouchableOpacity
                                        style={[
                                            ...
                                            ((viewState === 'instances' && !selectedInstance) || (viewState === 'batches' && !selectedBatch)) && { opacity: 0.5 }
                                        ]}
                                        onPress={handleAddItem}
                                        disabled={(viewState === 'instances' && !selectedInstance) || (viewState === 'batches' && !selectedBatch)}
                                    >
