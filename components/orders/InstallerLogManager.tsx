...
export default function InstallerLogManager({
  ...
  const handleDeleteLog = (index: number) => {
    Alert.alert('Confirmar Eliminación', '¿Estás seguro de eliminar esta entrada del registro?', [
  ...
  const handleSaveLog = () => {
    ...
    const newLog: InstallerLog = {
      timestamp:
        editingIndex !== null ? installerLogs[editingIndex].timestamp : new Date().toISOString(),
      log: logMessage.trim(),
    ...
    if (editingIndex !== null) {
      // Edit existing log
      newLogs = installerLogs.map((log: InstallerLog, i: number) =>
        i === editingIndex ? newLog : log
      );
    } else {
      // Add new log
              ...
              <View key={actualIndex} style={styles.logCard}>
                <View style={styles.logHeader}>
                  <View style={[styles.statusBadge, { backgroundColor: statusConfig.hexBgColor }]}>
                    <Text style={[styles.statusText, { color: statusConfig.hexColor }]}>
                      {statusConfig.label}
                    </Text>
                  </View>
            ...
            <View style={styles.formGroup}>
              ...
              <View style={styles.statusDisplay}>
                <View
                  style={[
                    styles.statusBadge,
                    { backgroundColor: getStatusConfig(currentStatus).color },
                  ]}>
