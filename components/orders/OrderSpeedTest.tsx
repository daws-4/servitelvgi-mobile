...
export default function OrderSpeedTest({
  orderId,
  existingTest,
  onTestSaved,
  readOnly = false,
  ...
  const canSave =
    results.status === 'Finalizado' && results.download !== '0' && results.upload !== '0';
  ...
  const handleSaveResults = async () => {
    ...
    try {
      ...
      console.log(
        '[OrderSpeedTest] 💾 Saving results. Current values:',
  ...
  if (existingTest?.downloadSpeed && !canSave && results.status === 'Inactivo' && !showNewTest) {
    return (
      <View style={styles.savedContainer}>
        ...
        {!readOnly && (
          <TouchableOpacity style={styles.retryButton} onPress={() => setShowNewTest(true)}>
