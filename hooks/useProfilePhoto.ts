...
interface UseProfilePhotoReturn {
  ...
  deleting: boolean;
  error: string | null;
  uploadPhoto: (userId: string, imageUri: string) => Promise<{ success: boolean; imageUrl?: string; error?: string }>;
...
export const useProfilePhoto = (): UseProfilePhotoReturn => {
  ...
  const uploadPhoto = useCallback(async (
    ...
    try {
      ...
      const errorMsg = err.response?.data?.error || err.message || 'Error al subir la foto de perfil';
  ...
  const deletePhoto = useCallback(async (
    ...
    try {
      ...
      const errorMsg = err.response?.data?.error || err.message || 'Error al eliminar la foto de perfil';
