...
export const useEquipmentInstances = (): UseEquipmentInstancesReturn => {
  ...
  const fetchInstances = useCallback(
    async (inventoryId: string, status?: string, crewId?: string) => {
      ...
      try {
        ...
        if (crewId && data && data.length > 0) {
          ...
          data = data.filter((instance) => {
            ...
            console.log(
              `[useEquipmentInstances] Instance ${instance.uniqueId}: rawCrewId type=${typeof rawCrewId}, extracted=${instanceCrewId}, matches=${instanceCrewId === crewId}`
            );
            return instanceCrewId === crewId;
          });
          console.log(
            `[useEquipmentInstances] Filtered from ${beforeFilter} to ${data.length} instances`
          );
        }
