...
export default function OrderDetailScreen() {
  ...
  const fetchOrder = async () => {
    try {
      ...
      if (err.response?.status === 404 || err.status === 404) {
        Alert.alert('Orden no encontrada', 'La orden que intentas ver no existe o fue eliminada.', [
          {
  ...
  const canComplete = useMemo(() => {
    ...
    const hasInternetTest = !!(
      order.internetTest?.downloadSpeed && order.internetTest?.uploadSpeed
    );
    const hasLogs = (order.installerLog?.length || 0) > 0;
  ...
  const { getAssignmentTimeAgo, isAssignmentDelayed } = useMemo(() => {
    if (!order?.assignmentDate)
      return { getAssignmentTimeAgo: 'No registrada', isAssignmentDelayed: false };
      ...
      <SafeAreaView style={styles.container}>
        ...
        <ScrollView showsVerticalScrollIndicator contentContainerStyle={styles.scrollContent}>
          {/* 
                ...
                */}
          <TouchableOpacity activeOpacity={1} onPress={clearSelection} style={{ flex: 1 }}>
              ...
              {order.type === 'recuperacion' && !isReadOnly ? (
                ...
                <>
                  ...
                  {order.phones && order.phones.length > 0 ? (
                    ...
                    <ReadOnlyField label="Teléfono" value="No registrado" icon="phone" />
                  )}

                  <ReadOnlyField label="Email" value={order.email} icon="envelope" selectable />
                </>
              )}
            </View>
            ...
            <View style={styles.section}>
              ...
              <ReadOnlyField
                label="Tipo de Orden"
                value={
                  ORDER_TYPE_LABELS[order.type as OrderType] || order.type || 'No especificado'
                }
                icon="tag"
                selectable
              />
              ...
              {order.type === 'recuperacion' && !isReadOnly ? (
                ...
                <ReadOnlyField label="Nodo" value={order.node} icon="sitemap" selectable />
              )}
                  ...
                  {!isReadOnly ? (
                    <>
                      ...
                      <EditableField
                        ...
                        onChangeText={(text) =>
                          handleOrderFieldUpdate('remainingPorts', parseInt(text) || 0)
                        }
                      ...
                      <ReadOnlyField
                        ...
                        value={
                          order.etiqueta?.color
                            ? order.etiqueta.color.charAt(0).toUpperCase() +
                              order.etiqueta.color.slice(1)
                            : 'No registra'
                        }
                        icon="tag"
                      />
              ...
              {order.assignmentDate && !getStatusConfig(currentStatus).isTerminal && (
                <ReadOnlyField
                  ...
                  icon="clock-o"
                  valueStyle={
                    isAssignmentDelayed ? { color: '#ef4444', fontWeight: '700' } : undefined
                  }
