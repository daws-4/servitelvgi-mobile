...
export default function InventoryScreen() {
    ...
    return inventory.filter((assignedItem) => {
      ...
      if (searchText) {
        ...
        if (details.type === 'equipment' && details.instances) {
          const hasMatchingInstance = details.instances.some(
            (inst) =>
  ...
  return (
    <View className="flex-1 bg-slate-50">
      {/* Header with Summary */}
      <InventoryHeader>
        <InventorySummary totalItems={stats.totalItems} lowStockCount={stats.lowStockCount} />
      </InventoryHeader>

      {/* Search and Filters */}
      <View className="mt-6 gap-4 px-6 pb-4">
      ...
      <TouchableOpacity
        className="absolute right-6 h-14 w-14 items-center justify-center rounded-2xl shadow-lg"
        style={{
